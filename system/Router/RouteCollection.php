...
class RouteCollection implements RouteCollectionInterface
{
  ...
  public function getRoutes(?string $verb = null, bool $includeWildcard = true): array
  {
    ...
    if (isset($this->routes[$verb])) {
      ...
      $collection = $includeWildcard
        ? $this->routes[$verb] + ($this->routes["*"] ?? [])
        : $this->routes[$verb];
  ...
  public function setHTTPVerb(string $verb)
  {
    if ($verb !== "*" && $verb === strtolower($verb)) {
      @trigger_error(
        'Passing lowercase HTTP method "' .
          $verb .
          '" is deprecated.' .
          ' Use uppercase HTTP method like "' .
          strtoupper($verb) .
          '".',
        E_USER_DEPRECATED,
      );
  ...
  public function resource(string $name, ?array $options = null): RouteCollectionInterface
  {
    ...
    $id =
      $options["placeholder"] ?? ($this->placeholders[$this->defaultPlaceholder] ?? "(:segment)");

    // Make sure we capture back-references
    $id = "(" . trim($id, "()") . ")";

    $methods = isset($options["only"])
      ? (is_string($options["only"])
        ? explode(",", $options["only"])
        : $options["only"])
      : ["index", "show", "create", "update", "delete", "new", "edit"];

    if (isset($options["except"])) {
      $options["except"] = is_array($options["except"])
        ? $options["except"]
        : explode(",", $options["except"]);
