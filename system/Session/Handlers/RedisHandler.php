...
class RedisHandler extends BaseHandler
{
  ...
  public function __construct(SessionConfig $config, string $ipAddress)
  {
    ...
    $this->sessionExpiration =
      $config->expiration === 0 ? (int) ini_get("session.gc_maxlifetime") : $config->expiration;
  ...
  public function open($path, $name): bool
  {
    ...
    if (
      !$redis->connect($this->savePath["host"], $this->savePath["port"], $this->savePath["timeout"])
    ) {
  ...
  public function destroy($id): bool
  {
    if (isset($this->redis, $this->lockKey)) {
      if (($result = $this->redis->del($this->keyPrefix . $id)) !== 1) {
        $this->logger->debug(
          "Session: Redis::del() expected to return 1, got " .
