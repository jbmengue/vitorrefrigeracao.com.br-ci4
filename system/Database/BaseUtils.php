...
abstract class BaseUtils
{
  ...
  public function optimizeTable(string $tableName)
  {
    ...
    $query = $this->db->query(
      sprintf($this->optimizeTable, $this->db->escapeIdentifiers($tableName)),
    );
  ...
  public function repairTable(string $tableName)
  {
    ...
    $query = $this->db->query(
      sprintf($this->repairTable, $this->db->escapeIdentifiers($tableName)),
    );
    if (is_bool($query)) {
      return $query;
    }
   ...
   * @return string
   */
  public function getCSVFromResult(
    ...
    while ($row = $query->getUnbufferedRow("array")) {
      ...
      foreach ($row as $item) {
        $line[] =
          $enclosure .
          str_replace($enclosure, $enclosure . $enclosure, (string) $item) .
          $enclosure;
      }
  ...
  {
    foreach (
      ["root" => "root", "element" => "element", "newline" => "\n", "tab" => "\t"]
      as $key => $val
    ) {
      if (!isset($params[$key])) {
        $params[$key] = $val;
