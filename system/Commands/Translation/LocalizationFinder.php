...
class LocalizationFinder extends BaseCommand
{
    ...
    if (is_string($optionLocale)) {
      if (!in_array($optionLocale, config(App::class)->supportedLocales, true)) {
        CLI::error(
          'Error: "' .
            $optionLocale .
            '" is not supported. Supported locales: ' .
  ...
  private function process(string $currentDir, string $currentLocale): void
  {
    ...
    foreach ($languageFoundGroups as $langFileName) {
      ...
      $languageDiff = ArrayHelper::recursiveDiff(
        $foundLanguageKeys[$langFileName],
        $languageStoredKeys,
      );
      ...
      if ($this->showNew) {
        ...
        $newLanguageKeys = array_replace_recursive(
          $foundLanguageKeys[$langFileName],
          $languageStoredKeys,
        );
        ...
        if ($languageDiff !== []) {
          if (
            file_put_contents($languageFilePath, $this->templateFile($newLanguageKeys)) === false
          ) {
    ...
    if (!$this->showNew && $countNewKeys > 0) {
      CLI::write(
        "Note: You need to run your linting tool to fix coding standards issues.",
        "white",
        "red",
      );
    }
  ...
  private function findTranslationsInFile($file): array
  {
    ...
    foreach ($matches[1] as $phraseKey) {
      ...
      if (count($phraseKeys) === 1) {
        ...
        $foundLanguageKeys[$languageFileName] = array_replace_recursive(
   ...
   */
  private function writeIsVerbose(
    string $text = "",
  ...
  private function findLanguageKeysInFiles(array $files): array
  {
    ...
    foreach ($files as $file) {
      ...
      $foundLanguageKeys = array_replace_recursive(
        $findInFile["foundLanguageKeys"],
        $foundLanguageKeys,
      );
