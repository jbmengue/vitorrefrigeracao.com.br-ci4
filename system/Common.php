...
if (!function_exists("clean_path")) {
  ...
  function clean_path(string $path): string
  {
    ...
    return match (true) {
      str_starts_with($path, APPPATH) => "APPPATH" .
        DIRECTORY_SEPARATOR .
        substr($path, strlen(APPPATH)),
      str_starts_with($path, SYSTEMPATH) => "SYSTEMPATH" .
        DIRECTORY_SEPARATOR .
        substr($path, strlen(SYSTEMPATH)),
      str_starts_with($path, FCPATH) => "FCPATH" .
        DIRECTORY_SEPARATOR .
        substr($path, strlen(FCPATH)),
      defined("VENDORPATH") && str_starts_with($path, VENDORPATH) => "VENDORPATH" .
        DIRECTORY_SEPARATOR .
        substr($path, strlen(VENDORPATH)),
      str_starts_with($path, ROOTPATH) => "ROOTPATH" .
        DIRECTORY_SEPARATOR .
        substr($path, strlen(ROOTPATH)),
      default => $path,
    };
  }
}
    ...
    while ($cursor < $length) {
      if (preg_match("/\s+/A", $command, $match, 0, $cursor)) {
        ...
        throw new InvalidArgumentException(
          sprintf('Unable to parse input near "... %s ...".', substr($command, $cursor, 10)),
        );
...
if (!function_exists("force_https")) {
  ...
  function force_https(
    ...
    if (
      (ENVIRONMENT !== "testing" && (is_cli() || $request->isSecure())) ||
    ...
    // Set an HSTS header
    $response
      ->setHeader("Strict-Transport-Security", "max-age=" . $duration)
      ->redirect((string) $uri)
      ->setStatusCode(307)
      ->setBody("")
      ->getCookieStore()
...
if (!function_exists("view_cell")) {
  ...
  function view_cell(
    ...
    return service("viewcell")->render($library, $params, $ttl, $cacheName);
  }
}
