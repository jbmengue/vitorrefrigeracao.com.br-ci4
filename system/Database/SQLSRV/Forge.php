...
class Forge extends BaseForge
{
  ...
  public function __construct(BaseConnection $db)
  {
    ...
    $this->renameTableStr =
      "EXEC sp_rename [" . $this->db->escapeIdentifiers($this->db->schema) . ".%s] , %s ;";

    $this->dropConstraintStr =
      "ALTER TABLE " . $this->db->escapeIdentifiers($this->db->schema) . ".%s DROP CONSTRAINT %s";
    $this->dropIndexStr =
      "DROP INDEX %s ON " . $this->db->escapeIdentifiers($this->db->schema) . ".%s";
  }
  ...
  {
    if ($ifNotExists) {
      $sql = sprintf($this->createDatabaseIfStr, $dbName, $this->db->escapeIdentifier($dbName));
    } else {
      $sql = sprintf($this->createDatabaseStr, $this->db->escapeIdentifier($dbName));
    }
  ...
  protected function _alterTable(string $alterType, string $table, $processedFields)
  {
    ...
    if ($alterType === "DROP") {
      ...
      $fullTable =
        $this->db->escapeIdentifiers($this->db->schema) .
        "." .
      ...
      $fields = array_map(
        static fn($item): string => "COLUMN [" . trim($item) . "]",
        (array) $columnNamesToDrop,
      );
    ...
    if ($alterType === "ADD") {
      foreach ($processedFields as $field) {
        $sqls[] =
          $sql .
          ($field["_literal"] !== false ? $field["_literal"] : $this->_processColumn($field));
      }
    ...
    foreach ($processedFields as $field) {
      ...
      if (
        isset($field["null"]) &&
        ($field["null"] === false || $field["null"] === " NOT " . $this->null)
      ) {
        $nullable = false;
      }
      $sqls[] =
  ...
  protected function _dropIndex(string $table, object $indexData)
  {
    if ($indexData->type === "PRIMARY") {
      $sql =
        "ALTER TABLE [" . $this->db->schema . "].[" . $table . "] DROP [" . $indexData->name . "]";
    } else {
      $sql =
        "DROP INDEX [" . $indexData->name . "] ON [" . $this->db->schema . "].[" . $table . "]";
    }
  ...
  protected function _processIndexes(string $table, bool $asQuery = false): array
  {
    ...
    for ($i = 0, $c = count($this->keys); $i < $c; $i++) {
      ...
      $keyName = $this->db->escapeIdentifiers(
        $this->keys[$i]["keyName"] === ""
  ...
  protected function _processColumn(array $processedField): string
  {
    return $this->db->escapeIdentifiers($processedField["name"]) .
      (empty($processedField["new_name"])
        ? ""
        : " " . $this->db->escapeIdentifiers($processedField["new_name"])) .
      " " .
      $processedField["type"] .
      ($processedField["type"] === "text" ? "" : $processedField["length"]) .
  ...
  protected function _attributeType(array &$attributes)
  {
    ...
    switch (strtoupper($attributes["TYPE"])) {
      ...
      case "ENUM":
        ...
        $maxLength = max(
          array_map(static fn($value): int => strlen($value), $attributes["CONSTRAINT"]),
        );
