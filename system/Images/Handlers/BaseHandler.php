  ...
  protected function reproportion()
  {
    ...
    if ($this->masterDim !== "width" && $this->masterDim !== "height") {
      if ($this->width > 0 && $this->height > 0) {
        $this->masterDim =
          $this->image()->origHeight / $this->image()->origWidth - $this->height / $this->width < 0
            ? "width"
            : "height";
      } else {
    ...
    if ($this->masterDim === "width") {
      $this->height = (int) ceil(
        ($this->width * $this->image()->origHeight) / $this->image()->origWidth,
      );
    } else {
      $this->width = (int) ceil(
        ($this->image()->origWidth * $this->height) / $this->image()->origHeight,
      );
    }
  }
