...
class Parser extends View
{
  ...
  protected function parseConditionals(string $template): string
  {
    ...
    $pattern =
      "/" . $leftDelimiter . "\s*(if|elseif)\s*((?:\()?(.*?)(?:\))?)\s*" . $rightDelimiter . "/ums";
   ...
   * @param string $rightDelimiter
   */
  public function setConditionalDelimiters(
    $leftDelimiter = "{",
    $rightDelimiter = "}",
  ): RendererInterface {
  ...
  protected function replaceSingle($pattern, $content, $template, bool $escape = false): string
  {
    ...
    return preg_replace_callback(
      $pattern,
      function ($matches) use ($content, $escape): string {
   ...
   */
  protected function prepareReplacement(
    array $matches,
    string $replace,
    bool $escape = true,
  ): string {
  ...
  protected function applyFilters(string $replace, array $filters): string
  {
    ...
    foreach ($filters as $filter) {
      ...
      // Get our filter name
      $filter =
        $param !== [] ? trim(strtolower(substr($filter, 0, strpos($filter, "(")))) : trim($filter);
