...
class File extends SplFileInfo
{
   ...
   */
  public function getSizeByBinaryUnit(
   ...
   * @param positive-int $precision
   */
  public function getSizeByMetricUnit(
    FileSizeUnit $unit = FileSizeUnit::B,
    int $precision = 3,
  ): int|string {
  ...
  public function move(string $targetPath, ?string $name = null, bool $overwrite = false)
  {
    ...
    if (!@rename($oldName, $destination)) {
      ...
      throw FileException::forUnableToMove(
        $this->getBasename(),
        $targetPath,
    ...
    while (is_file($destination)) {
      ...
      if (str_contains($info["filename"], $delimiter)) {
        ...
        if (is_numeric(end($parts))) {
          ...
          $parts[] = ++$i;
          $destination =
            $info["dirname"] . DIRECTORY_SEPARATOR . implode($delimiter, $parts) . $extension;
        } else {
          $destination =
            $info["dirname"] .
            DIRECTORY_SEPARATOR .
  ...
  private function getSizeByUnitInternal(
    int $fileSizeBase,
    FileSizeUnit $unit,
    int $precision,
  ): int|string {
