...
class Encryption
{
  ...
  public function __construct(?EncryptionConfig $config = null)
  {
    ...
    $this->handlers = [
      ...
      "Sodium" =>
        extension_loaded("sodium") && version_compare(SODIUM_LIBRARY_VERSION, "1.0.14", ">="),
    ];

    if (
      !in_array($this->driver, $this->drivers, true) ||
      (array_key_exists($this->driver, $this->handlers) && !$this->handlers[$this->driver])
    ) {
