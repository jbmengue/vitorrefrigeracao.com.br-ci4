    ...
    <div id="ci-vars" class="tab">
        ...
        <?php if (isset($vars["varData"])): ?>
            <?php foreach ($vars["varData"] as $heading => $items): ?>
                ...
                <a class="debug-bar-vars" data-toggle="datatable" data-table="<?= strtolower(
                  str_replace(" ", "-", $heading),
                ) ?>">
                    <h2><?= $heading ?></h2>
                </a>
        ...
        <h2>Response
            <span>( <?= $vars["response"]["statusCode"] .
              " - " .
              $vars["response"]["reason"] ?> )</span>
        </h2>

        <?php if (
          isset($vars["response"]["headers"]) &&
          ($headers = $vars["response"]["headers"])
        ): ?>
