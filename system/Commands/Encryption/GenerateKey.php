...
class GenerateKey extends BaseCommand
{
  ...
  protected $options = [
    ...
    "--length" =>
      "The length of the random string that should be returned in bytes. Defaults to 32.",
    "--prefix" =>
      "Prefix to prepend to encoded key (either hex2bin or base64). Defaults to hex2bin.",
  ...
  public function run(array $params)
  {
    ...
    if (in_array($prefix, [null, true], true)) {
      ...
      $prefix = CLI::prompt(
        "Please provide a valid prefix to use.",
        ["hex2bin", "base64"],
        "required",
      ); // @codeCoverageIgnore
    }
  ...
  protected function confirmOverwrite(array $params): bool
  {
    return array_key_exists("force", $params) ||
      CLI::getOption("force") ||
      CLI::prompt("Overwrite existing key?", ["n", "y"]) === "y";
  }
