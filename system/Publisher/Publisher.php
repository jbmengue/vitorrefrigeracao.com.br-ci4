...
class Publisher extends FileCollection
{
   ...
   * @return list<self>
   */
  final public static function discover(
    string $directory = "Publishers",
    string $namespace = "",
  ): array {
  ...
  public function publish(): bool
  {
    ...
    if ($this->source === ROOTPATH && $this->destination === FCPATH) {
      throw new RuntimeException(
        "Child classes of Publisher should provide their own publish method or a source and destination.",
      );
    }
  ...
  final public function getScratch(): string
  {
    if ($this->scratch === null) {
      ...
      $this->scratch = realpath($this->scratch)
        ? realpath($this->scratch) . DIRECTORY_SEPARATOR
        : $this->scratch;
    }
