...
class IncomingRequest extends Request
{
  ...
  public function isSecure(): bool
  {
    ...
    if (
      $this->hasHeader("X-Forwarded-Proto") &&
      $this->header("X-Forwarded-Proto")->getValue() === "https"
    ) {
      return true;
    }
  ...
  public function getJsonVar($index = null, bool $assoc = false, ?int $filter = null, $flags = null)
  {
    ...
    if (
      $filter !== FILTER_DEFAULT ||
      ((is_numeric($flags) && $flags !== 0) || (is_array($flags) && $flags !== []))
    ) {
      if (is_array($data)) {
        ...
        array_walk_recursive($data, static function (&$val) use ($filter, $flags): void {
          ...
          if (
            in_array($valType, ["int", "integer", "float", "double", "bool", "boolean"], true) &&
            $val !== false
          ) {
            settype($val, $valType);
          }
        ...
        if (
          in_array($dataType, ["int", "integer", "float", "double", "bool", "boolean"], true) &&
          $data !== false
        ) {
