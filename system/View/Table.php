...
class Table
{
  ...
  public function addRow()
  {
    ...
    if ($this->syncRowsWithHeading && $this->heading !== []) {
      ...
      $tmpRow = array_filter(
        $tmpRow,
        static fn($k): bool => array_key_exists($k, $keyIndex),
      ...
      $tmpRow = array_merge(
        $tmpRow,
        array_map(fn($v): array => ["data" => $this->emptyCells], $missingKeys),
      );
  ...
  public function generate($tableData = null)
  {
    ...
    if ($this->heading !== []) {
      ...
      if (
        preg_match("/(<)(td|th)(?=\h|>)/i", $this->template["heading_cell_start"], $matches) === 1
      ) {
        $headerTag = $matches[0];
      }
    ...
    if ($this->footing !== []) {
      ...
      $out .=
        $this->template["tfoot_open"] .
        $this->newline .
        $this->template["footing_row_start"] .
        $this->newline;
