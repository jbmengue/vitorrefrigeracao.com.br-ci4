...
class Forge extends BaseForge
{
      ...
      if ($processedFields[$i]["_literal"] !== false) {
        ...
        if (!empty($processedFields[$i]["comment"])) {
          $sqls[] =
            ...
            "." .
            $this->db->escapeIdentifiers($processedFields[$i]["name"]) .
            " IS " .
            $processedFields[$i]["comment"];
        }
        ...
        if ($alterType === "MODIFY" && !empty($processedFields[$i]["new_name"])) {
          $sqls[] =
            ...
            $this->db->escapeIdentifiers($processedFields[$i]["name"]) .
            " TO " .
            $this->db->escapeIdentifiers($processedFields[$i]["new_name"]);
        }
  ...
  protected function _processColumn(array $processedField): string
  {
    ...
    if (
      ...
      str_starts_with($processedField["length"], "('")
    ) {
      $constraint =
        " CHECK(" .
      ...
      $processedField["length"] =
        "(" .
        max(
          array_map(mb_strlen(...), explode("','", mb_substr($processedField["length"], 2, -2))),
        ) .
        ")" .
        $constraint;
    } elseif (
      isset($this->primaryKeys["fields"]) &&
      count($this->primaryKeys["fields"]) === 1 &&
      $processedField["name"] === $this->primaryKeys["fields"][0]
    ) {
