...
class Forge
{
  ...
  public function createTable(string $table, bool $ifNotExists = false, array $attributes = [])
  {
    ...
    if (($result = $this->db->query($sql)) !== false) {
      if (
        isset($this->db->dataCache["table_names"]) &&
        !in_array($table, $this->db->dataCache["table_names"], true)
      ) {
  ...
  protected function _createTable(string $table, bool $ifNotExists, array $attributes)
  {
    ...
    for ($i = 0, $c = count($processedFields); $i < $c; $i++) {
      $processedFields[$i] =
        $processedFields[$i]["_literal"] !== false
  ...
  public function dropTable(string $tableName, bool $ifExists = false, bool $cascade = false)
  {
    ...
    if (
      ($query = $this->_dropTable($this->db->DBPrefix . $tableName, $ifExists, $cascade)) === true
    ) {
      return true;
    }
  ...
  protected function _alterTable(string $alterType, string $table, $processedFields)
  {
    ...
    if ($alterType === "DROP") {
      ...
      $columnNamesToDrop = array_map(
        fn($field): string => "DROP COLUMN " . $this->db->escapeIdentifiers(trim($field)),
        $columnNamesToDrop,
      );
    ...
    foreach ($processedFields as $field) {
      $sqls[] =
        $sql . ($field["_literal"] !== false ? $field["_literal"] : $this->_processColumn($field));
    }
  ...
  protected function _processPrimaryKeys(string $table, bool $asQuery = false): string
  {
    ...
    if (isset($this->primaryKeys["fields"]) && $this->primaryKeys["fields"] !== []) {
      if ($asQuery) {
        $sql .=
          "ALTER TABLE " . $this->db->escapeIdentifiers($this->db->DBPrefix . $table) . " ADD ";
      } else {
        $sql .= ",\n\t";
      }
      $sql .=
        "CONSTRAINT " .
        $this->db->escapeIdentifiers(
          $this->primaryKeys["keyName"] === "" ? "pk_" . $table : $this->primaryKeys["keyName"],
        ) .
        " PRIMARY KEY(" .
    ...
    foreach ($this->foreignKeys as $index => $fkey) {
      ...
      $nameIndex =
        $fkey["fkName"] !== ""
          ...
          : $table .
            "_" .
            implode("_", $fkey["field"]) .
            ($this->db->DBDriver === "OCI8" ? "_fk" : "_foreign");

      $nameIndexFilled = $this->db->escapeIdentifiers($nameIndex);
      $foreignKeyFilled = implode(", ", $this->db->escapeIdentifiers($fkey["field"]));
      $referenceTableFilled = $this->db->escapeIdentifiers(
        $this->db->DBPrefix . $fkey["referenceTable"],
      );
      ...
      $sqls[$index] .= sprintf(
        ...
        $nameIndexFilled,
        $foreignKeyFilled,
        $referenceTableFilled,
        $referenceFieldFilled,
      );
      ...
      if (
        ...
        $fkey["onUpdate"] !== false &&
        in_array($fkey["onUpdate"], $this->fkAllowActions, true)
      ) {
