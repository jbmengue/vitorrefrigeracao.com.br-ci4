...
class Session implements SessionInterface
{
    ...
    $this->cookie = (new Cookie($this->config->cookieName, "", [
      "expires" =>
        $this->config->expiration === 0
          ? 0
          : Time::now()->getTimestamp() + $this->config->expiration,
      "path" => $cookie->path,
  ...
  public function start()
  {
    ...
    if (session_status() === PHP_SESSION_ACTIVE) {
      $this->logger->warning(
        'Session: Sessions is enabled, and one exists. Please don\'t $session->start();',
      );
    ...
    if (
      ...
      (!is_string($_COOKIE[$this->config->cookieName]) ||
        preg_match("#\A" . $this->sidRegexp . "\z#", $_COOKIE[$this->config->cookieName]) !== 1)
    ) {
    ...
    elseif (
      isset($_COOKIE[$this->config->cookieName]) &&
      $_COOKIE[$this->config->cookieName] === session_id()
    ) {
      $this->setCookie();
    }
    ...
    $this->initVars();
    $this->logger->debug(
      "Session: Class initialized using '" . $this->config->driver . "' driver.",
    );
  ...
  {
    $expiration =
      $this->config->expiration === 0 ? 0 : Time::now()->getTimestamp() + $this->config->expiration;
