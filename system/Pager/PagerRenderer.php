...
class PagerRenderer
{
  ...
  public function links(): array
  {
    ...
    for ($i = $this->first; $i <= $this->last; $i++) {
      $uri =
        $this->segment === 0
          ? $uri->addQuery($this->pageSelector, $i)
          : $uri->setSegment($this->segment, $i);
      $links[] = [
  ...
  protected function updatePages(?int $count = null)
  {
    ...
    $this->last =
      $this->current + $count <= $this->pageCount
        ? $this->current + $count
        : (int) $this->pageCount;
  }
  ...
  protected function updatePerPages(): void
  {
    ...
    $this->perPageStart =
      $this->current === 1 ? 1 : $this->perPage * $this->current - $this->perPage + 1;
