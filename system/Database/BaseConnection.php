...
abstract class BaseConnection implements ConnectionInterface
{
  ...
  public function protectIdentifiers(
    ...
    if (is_array($item)) {
      ...
      foreach ($item as $k => $v) {
        $escapedArray[$this->protectIdentifiers($k)] = $this->protectIdentifiers(
          $v,
          $prefixSingle,
          $protectIdentifiers,
          $fieldExists,
  ...
  private function protectDotItem(
    string $item,
    string $alias,
  ...
  public function escapeIdentifier($item): string
  {
    ...
    return $this->escapeChar .
      str_replace($this->escapeChar, $this->escapeChar . $this->escapeChar, $item) .
      $this->escapeChar;
  }
  ...
  public function escapeIdentifiers($item)
  {
    ...
    foreach ($this->reservedIdentifiers as $id) {
      ...
      if (str_contains($item, "." . $id)) {
        return preg_replace(
          "/" .
            $this->pregEscapeChar[0] .
            "?([^" .
            $this->pregEscapeChar[1] .
            "\.]+)" .
            $this->pregEscapeChar[1] .
            "?\./i",
  ...
  public function isWriteType($sql): bool
  {
    return (bool) preg_match(
      '/^\s*(WITH\s.+(\s|[)]))?"?(SET|INSERT|UPDATE|DELETE|REPLACE|CREATE|DROP|TRUNCATE|LOAD|COPY|ALTER|RENAME|GRANT|REVOKE|LOCK|UNLOCK|REINDEX|MERGE)\s(?!.*\sRETURNING\s)/is',
      $sql,
    );
  }
