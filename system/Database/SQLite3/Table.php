...
class Table
{
  ...
  public function fromTable(string $table)
  {
    ...
    if (
      $primaryIndexes !== [] &&
      count($primaryIndexes) > 1 &&
      array_key_exists("primary", $this->keys)
    ) {
  ...
  public function dropPrimaryKey(): Table
  {
    $primaryIndexes = array_filter(
      $this->keys,
      static fn($index): bool => strtolower($index["type"]) === "primary",
    );
  ...
  protected function createTable()
  {
    ...
    $this->keys = array_filter(
      $this->keys,
      static fn($index): bool => count(array_intersect($index["fields"], $fieldNames)) ===
        count($index["fields"]),
    );
