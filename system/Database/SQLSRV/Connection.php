...
class Connection extends BaseConnection
{
  ...
  {
    $charset = in_array(strtolower($this->charset), ["utf-8", "utf8"], true)
      ? "UTF-8"
      : SQLSRV_ENC_CHAR;
  ...
  public function getAllErrorMessages(): string
  {
    ...
    foreach (sqlsrv_errors() as $error) {
      $errors[] =
        $error["message"] . " SQLSTATE: " . $error["SQLSTATE"] . ", code: " . $error["code"];
    }
  ...
  protected function _indexData(string $table): array
  {
    ...
    foreach ($query as $row) {
      ...
      if (str_contains($row->index_description, "primary key located on")) {
        ...
        $obj->type = str_contains($row->index_description, "nonclustered, unique")
          ? "UNIQUE"
          : "INDEX";
      }
  ...
  public function error(): array
  {
    ...
    if (isset($sqlsrvError["SQLSTATE"])) {
      $error["code"] = isset($sqlsrvError["code"])
        ? $sqlsrvError["SQLSTATE"] . "/" . $sqlsrvError["code"]
  ...
  {
    $stmt =
      $this->scrollable === false || $this->isWriteType($sql)
  ...
  public function getVersion(): string
  {
    ...
    return isset($info["SQLServerVersion"])
      ? ($this->dataCache["version"] = $info["SQLServerVersion"])
      : "";
  }
