...
class LocalizationSync extends BaseCommand
{
  ...
  private function process(string $originalLocale, string $targetLocale): int
  {
    ...
    if (!is_dir($originalLocaleDir)) {
      CLI::error('Error: The "' . clean_path($originalLocaleDir) . '" directory was not found.');
    ...
    foreach ($files as $originalLanguageFile) {
      ...
      $targetLanguageFile =
        $targetLocaleDir . DIRECTORY_SEPARATOR . $originalLanguageFile->getFilename();
      ...
      $targetLanguageKeys = $this->mergeLanguageKeys(
        $originalLanguageKeys,
        $targetLanguageKeys,
        $originalLanguageFile->getBasename(".php"),
      );
   ...
   */
  private function mergeLanguageKeys(
    array $originalLanguageKeys,
    ...
    foreach ($originalLanguageKeys as $key => $value) {
      ...
      if (is_string($value)) {
        ...
        $mergedLanguageKeys[$key] = $this->mergeLanguageKeys(
          $value,
        ...
        throw new LogicException(
          'Value for the key "' .
            $placeholderValue .
            '" is of the wrong type. Only "array" or "string" is allowed.',
        );
      }
    }
