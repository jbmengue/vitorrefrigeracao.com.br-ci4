...
class Connection extends BaseConnection
{
  ...
  public function insertID(): int
  {
    ...
    $query = $this->query(
      "SELECT DATA_DEFAULT FROM USER_TAB_COLUMNS WHERE TABLE_NAME = ? AND COLUMN_NAME = ?",
      [$this->lastInsertedTableName, $primaryColumnName],
    )->getRow();
  ...
  protected function buildDSN()
  {
    ...
    $isEasyConnectableHostName =
      $this->hostname !== "" &&
      !str_contains($this->hostname, "/") &&
      !str_contains($this->hostname, ":");
    $easyConnectablePort =
      $this->port !== "" && ctype_digit((string) $this->port) ? ":" . $this->port : "";
     ...
     */
    if (
      preg_match($this->validDSNs["ec"], $this->hostname) ||
      preg_match($this->validDSNs["in"], $this->hostname)
    ) {
      $this->DSN = $this->hostname;
  ...
  public function getDatabase(): string
  {
    ...
    return $this->query("SELECT DEFAULT_TABLESPACE FROM USER_USERS")->getRow()
      ->DEFAULT_TABLESPACE ?? "";
  }
