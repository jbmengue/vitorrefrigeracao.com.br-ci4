...
trait TimeTrait
{
  ...
  {
    $this->locale =
      $locale !== null && $locale !== "" && $locale !== "0" ? $locale : Locale::getDefault();
  ...
  public static function create(
    ...
    return new static(
      date("Y-m-d H:i:s", strtotime("{$year}-{$month}-{$day} {$hour}:{$minutes}:{$seconds}")),
      $timezone,
      $locale,
    );
  }
  ...
  public function equals($testTime, ?string $timezone = null): bool
  {
    ...
    $ourTime = $this->toDateTime()->setTimezone(new DateTimeZone("UTC"))->format("Y-m-d H:i:s.u");
  ...
  public function sameAs($testTime, ?string $timezone = null): bool
  {
    if ($testTime instanceof DateTimeInterface) {
      ...
      $timezone =
        $timezone !== null && $timezone !== "" && $timezone !== "0" ? $timezone : $this->timezone;
      $timezone = $timezone instanceof DateTimeZone ? $timezone : new DateTimeZone($timezone);
