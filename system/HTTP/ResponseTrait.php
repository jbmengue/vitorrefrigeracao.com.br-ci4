...
trait ResponseTrait
{
  ...
  public function sendHeaders()
  {
    ...
    foreach ($this->headers() as $name => $value) {
      if ($value instanceof Header) {
        header($name . ": " . $value->getValueLine(), true, $this->getStatusCode());
      } else {
        $replace = true;

        foreach ($value as $header) {
          header($name . ": " . $header->getValueLine(), $replace, $this->getStatusCode());
          $replace = false;
        }
      }
    }
    ...
    if (
      ...
      if (
        ...
        if ($_SERVER["REQUEST_METHOD"] === Method::GET) {
          ...
          in_array($_SERVER["REQUEST_METHOD"], [Method::POST, Method::PUT, Method::DELETE], true)
        ) {
  ...
  public function setCookie(
    ...
    if (is_array($name)) {
      ...
      foreach (
        ["samesite", "value", "expire", "domain", "path", "prefix", "secure", "httponly", "name"]
        as $item
      ) {
        if (isset($name[$item])) {
          ${$item} = $name[$item];
