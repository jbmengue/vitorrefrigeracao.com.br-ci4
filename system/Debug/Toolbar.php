...
class Toolbar
{
  ...
  public function __construct(ToolbarConfig $config)
  {
    ...
    foreach ($config->collectors as $collector) {
      if (!class_exists($collector)) {
        log_message(
          "critical",
          "Toolbar collector does not exist (" .
            $collector .
            ")." .
  ...
  public function run(
    ...
    if (isset($_SESSION)) {
      foreach ($_SESSION as $key => $value) {
        ...
        $data["vars"]["session"][esc($key)] = is_string($value)
          ? esc($value)
    ...
    foreach ($request->getGet() as $name => $value) {
      $data["vars"]["get"][esc($name)] = is_array($value)
        ? "<pre>" . esc(print_r($value, true)) . "</pre>"
        : esc($value);
    }

    foreach ($request->getPost() as $name => $value) {
      $data["vars"]["post"][esc($name)] = is_array($value)
        ? "<pre>" . esc(print_r($value, true)) . "</pre>"
        : esc($value);
    }
  ...
  protected function renderTimeline(
    ...
    return $this->renderTimelineRecursive(
      $rows,
      $startTime,
      $segmentCount,
      $segmentDuration,
  ...
  protected function renderTimelineRecursive(
    ...
    foreach ($rows as $row) {
      ...
      if ($hasChildren || $isQuery) {
        $output .=
          '<tr class="timeline-parent' .
          ($open ? " timeline-parent-open" : "") .
          '" id="timeline-' .
          $styleCount .
          '_parent" data-toggle="childrows" data-child="timeline-' .
          $styleCount .
          '">';
      } else {
        $output .= "<tr>";
      ...
      $output .=
        '<td class="' .
        ($isChild ? "debug-bar-width30" : "") .
        " debug-bar-level-" .
        $level .
        '" >' .
        ($hasChildren || $isQuery ? "<nav></nav>" : "") .
        $row["name"] .
        "</td>";
      $output .=
        '<td class="' . ($isChild ? "debug-bar-width10" : "") . '">' . $row["component"] . "</td>";
      $output .=
        '<td class="' .
        ($isChild ? "debug-bar-width10 " : "") .
        'debug-bar-alignRight">' .
        number_format($row["duration"] * 1000, 2) .
        " ms</td>";
      ...
      $output .=
        "<span class='timer debug-bar-timeline-{$styleCount}' title='" .
      ...
      if ($hasChildren || $isQuery) {
        ...
        if ($isQuery) {
          ...
          $output .= "<tr>";
          $output .=
            '<td class="query-container debug-bar-level-' .
            ($level + 1) .
            '" >' .
            $row["query"] .
            "</td>";
          ...
          $output .= $this->renderTimelineRecursive(
            $row["children"],
            $startTime,
            $segmentCount,
            $segmentDuration,
  ...
  protected function collectTimelineData($collectors): array
  {
    ...
    // Sort it
    $sortArray = [
      array_column($data, "start"),
      SORT_NUMERIC,
      SORT_ASC,
      array_column($data, "duration"),
      SORT_NUMERIC,
      SORT_DESC,
      &$data,
    ];
  ...
  public function prepare(?RequestInterface $request = null, ?ResponseInterface $response = null)
  {
    ...
    if (CI_DEBUG && !is_cli()) {
      ...
      $script =
        ...
        "<script " .
        csp_script_nonce() .
        ' id="debugbar_dynamic_script"></script>' .
        "<style " .
        csp_style_nonce() .
        ' id="debugbar_dynamic_style"></style>' .
        $kintScript .
        PHP_EOL;
  ...
  protected function format(string $data, string $format = "html"): string
  {
    ...
    if (
      preg_match(
        "/\d+\.\d{6}/s",
        (string) service("request")->getGet("debugbar_time"),
        $debugbarTime,
      )
    ) {
      $history = new History();
