...
if (!function_exists("form_open")) {
  ...
  function form_open(string $action = "", $attributes = [], array $hidden = []): string
  {
    ...
    if (
      ...
      str_contains($action, base_url()) &&
      !str_contains(strtolower($form), strtolower('method="get"'))
    ) {
...
if (!function_exists("form_input")) {
  ...
  function form_input($data = "", string $value = "", $extra = "", string $type = "text"): string
  {
    ...
    return "<input " .
      parse_form_attributes($data, $defaults) .
      stringify_attributes($extra) .
      _solidus() .
      ">\n";
  }
}
...
if (!function_exists("form_textarea")) {
  ...
  function form_textarea($data = "", string $value = "", $extra = ""): string
  {
    ...
    if (
      (is_array($extra) && array_key_exists("rows", $extra)) ||
      (is_string($extra) && str_contains(strtolower(preg_replace("/\s+/", "", $extra)), "rows="))
    ) {
      unset($defaults["rows"]);
    }

    if (
      (is_array($extra) && array_key_exists("cols", $extra)) ||
      (is_string($extra) && str_contains(strtolower(preg_replace("/\s+/", "", $extra)), "cols="))
    ) {
      unset($defaults["cols"]);
    }

    return "<textarea " .
      rtrim(parse_form_attributes($data, $defaults)) .
      stringify_attributes($extra) .
      ">" .
...
if (!function_exists("form_dropdown")) {
  ...
  function form_dropdown($data = "", $options = [], $selected = [], $extra = ""): string
  {
    ...
    $extra = stringify_attributes($extra);
    $multiple =
      count($selected) > 1 && !str_contains(strtolower($extra), "multiple")
        ? ' multiple="multiple"'
        : "";
    $form =
      "<select " . rtrim(parse_form_attributes($data, $defaults)) . $extra . $multiple . ">\n";
