...
class CLI
{
  ...
  public static function promptByMultipleKeys(string $text, array $options): array
  {
    ...
    if ($opts === []) {
      ...
      $extraOutput =
        "You can specify multiple values separated by commas." . PHP_EOL . $extraOutput;
    }
  ...
  private static function getColoredText(
    string $text,
    string $foreground,
    ?string $background,
    ?string $format,
  ...
  public static function generateDimensions()
  {
    try {
      if (is_windows()) {
        ...
        if (
          getenv("TERM") ||
          (($shell = getenv("SHELL")) && preg_match('/(?:bash|zsh)(?:\.exe)?$/', $shell))
        ) {
  ...
  public static function showProgress($thisStep = 1, int $totalSteps = 10)
  {
    ...
    if ($thisStep !== false) {
      ...
      static::fwrite(
        STDOUT,
        "[\033[32m" . str_repeat("#", $step) . str_repeat(".", 10 - $step) . "\033[0m]",
      );
