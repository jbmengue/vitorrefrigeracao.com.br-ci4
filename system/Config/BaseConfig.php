...
class BaseConfig
{
  ...
  protected function initEnvValue(&$property, string $name, string $prefix, string $shortPrefix)
  {
    if (is_array($property)) {
      ...
      ($value = $this->getEnvValue($name, $prefix, $shortPrefix)) !== false &&
      $value !== null
    ) {
      if ($value === "false") {
        $value = false;
      ...
      if (static::$discovering) {
        throw new ConfigException(
          "During Auto-Discovery of Registrars," .
            ' "' .
            static::class .
            '" executes Auto-Discovery again.' .
            ' "' .
            clean_path(static::$registrarFile) .
            '" seems to have bad code.',
        );
      }
    ...
    foreach (static::$registrars as $callable) {
      ...
      if (!is_array($properties)) {
        throw new RuntimeException(
          "Registrars must return an array of properties and their values.",
        );
      }
