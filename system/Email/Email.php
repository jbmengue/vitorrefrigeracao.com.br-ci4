...
class Email
{
  ...
  protected function buildMessage()
  {
    ...
    $this->finalBody =
      $this->getProtocol() === "mail" ? $body : $hdr . $this->newline . $this->newline . $body;
  }
  ...
  protected function appendAttachments(&$body, $boundary, $multipart = null)
  {
    foreach ($this->attachments as $attachment) {
      ...
      $body .=
        ...
        $this->newline .
        (isset($attachment["cid"]) && $attachment["cid"] !== ""
          ? "Content-ID: <" . $attachment["cid"] . ">" . $this->newline
          : "") .
        $this->newline .
        $attachment["content"] .
        $this->newline;
  ...
  protected function prepQEncoding($str)
  {
    ...
    for ($i = 0, $length = static::strlen($output); $i < $chars; $i++) {
      $chr =
        $this->charset === "UTF-8" && extension_loaded("iconv")
          ? "=" .
            implode(
              "=",
              str_split(strtoupper(bin2hex(iconv_substr($str, $i, 1, $this->charset))), 2),
            )
  ...
  protected function spoolEmail()
  {
    ...
    if (!$success) {
      $message = lang(
        "Email.sendFailure" . ($protocol === "mail" ? "PHPMail" : $upperFirstProtocol),
      );
  ...
  protected function sendWithSendmail()
  {
    ...
    if (
      !function_usable("popen") ||
      false === ($fp = @popen($this->mailPath . " -oi " . $from . " -t", "w"))
    ) {
      return false;
    }
  ...
  protected function sendCommand($cmd, $data = "")
  {
    switch ($cmd) {
      ...
      case "to":
        if ($this->DSN) {
          $this->sendData(
            "RCPT TO:<" . $data . "> NOTIFY=SUCCESS,DELAY,FAILURE ORCPT=rfc822;" . $data,
          );
        } else {
  ...
  protected function sendData($data)
  {
    ...
    for (
      $written = $timestamp = 0, $length = static::strlen($data);
      $written < $length;
      $written += $result
    ) {
