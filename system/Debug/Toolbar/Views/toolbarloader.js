...
function loadDoc(time) {
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState === 4 && this.status === 200) {
      ...
      console.log(
        'CodeIgniter DebugBar: File "WRITEPATH/debugbar/debugbar_' + time + '" not found.'
      );
    }
  };
      ...
      if (
        ...
        if (realXHR.getAllResponseHeaders().indexOf("Debugbar-Time") >= 0) {
          ...
          if (debugbarTime) {
            ...
            if (h2) {
              h2.innerHTML =
                'History <small>You have new debug data.</small> <button id="ci-history-update">Update</button>';
              document.querySelector('a[data-tab="ci-history"] > span > .badge').className +=
                " active";
              document.getElementById("ci-history-update").addEventListener(
                "click",
                function () {
