...
class BaseBuilder
{
  ...
  protected function maxMinAvgSum(string $select = "", string $alias = "", string $type = "MAX")
  {
    ...
    $sql =
      $type .
      "(" .
      $this->db->protectIdentifiers(trim($select)) .
      ") AS " .
      $this->db->escapeIdentifiers(trim($alias));
    ...
    if (!$this->hasOperator($cond)) {
      ...
      foreach ($conditions as $i => $condition) {
        ...
        $cond .= preg_match(
          ...
          ? $match[1] .
            $this->db->protectIdentifiers($match[2]) .
            $operator .
            $this->db->protectIdentifiers($match[3])
          : $condition;
      }
    }
   ...
   * @return $this
   */
  protected function whereHaving(
    string $qbKey,
    $key,
    $value = null,
    string $type = "AND ",
    ...
    foreach ($keyValue as $k => $v) {
      ...
      if ($rawSqlOnly) {
        ...
        preg_match('/\s*(!?=|<>|IS(?:\s+NOT)?)\s*$/i', $k, $match, PREG_OFFSET_CAPTURE)
      ) {
  ...
  protected function _whereIn(
    ...
    if ($key === null || $key === "") {
      throw new InvalidArgumentException(
        sprintf('%s() expects $key to be a non-empty string', debug_backtrace(0, 2)[1]["function"]),
      );
    }
    ...
    if ($values === null || (!is_array($values) && !$this->isSubquery($values))) {
      throw new InvalidArgumentException(
        sprintf(
          '%s() expects $values to be of type array or closure',
   ...
   * @return $this
   */
  protected function groupStartPrepare(
    string $not = "",
    string $type = "AND ",
    string $clause = "QBWhere",
  ) {
  ...
  protected function _limit(string $sql, bool $offsetIgnore = false): string
  {
    return $sql .
      " LIMIT " .
      ($offsetIgnore === false && $this->QBOffset ? $this->QBOffset . ", " : "") .
      $this->QBLimit;
  }
  ...
  public function countAll(bool $reset = true)
  {
    ...
    $sql =
      $this->countString .
      $this->db->escapeIdentifiers("numrows") .
      " FROM " .
  ...
  public function countAllResults(bool $reset = true)
  {
    ...
    if ($this->QBDistinct === true || !empty($this->QBGroupBy)) {
      ...
      $sql =
        $this->countString .
        $this->db->protectIdentifiers("numrows") .
        "\nFROM (\n" .
        $this->compileSelect() .
  ...
  public function setData($set, ?bool $escape = null, string $alias = "")
  {
    ...
    foreach ($set as $row) {
      ...
      if (
        ...
        array_diff(array_keys($row), $keys) !== []
      ) {
  ...
  protected function _upsertBatch(string $table, array $keys, array $values): string
  {
    ...
    if ($sql === "") {
      $updateFields =
        $this->QBOptions["updateFields"] ??
        ($this->updateFields($keys)->QBOptions["updateFields"] ?? []);
      ...
      $sql =
        ...
        implode(
          ",\n",
          array_map(
            static fn($key, $value): string => $table .
              "." .
              $key .
              ($value instanceof RawSql ? " = " . $value : " = VALUES(" . $value . ")"),
    ...
    if (!empty($set)) {
      ...
      foreach ($set as $key => $value) {
        ...
        if ($ignore === null || !in_array($key, $ignore, true)) {
          if ($addToDefault) {
            $this->QBOptions["updateFieldsAdditional"][
              $this->db->protectIdentifiers($key)
            ] = $value;
          } else {
      ...
      if (
        $addToDefault === false &&
        isset($this->QBOptions["updateFieldsAdditional"], $this->QBOptions["updateFields"])
      ) {
        $this->QBOptions["updateFields"] = array_merge(
          $this->QBOptions["updateFields"],
          $this->QBOptions["updateFieldsAdditional"],
        );
    ...
    if ($limit !== null) {
      if (!$this->canLimitWhereUpdates) {
        throw new DatabaseException(
          "This driver does not allow LIMITs on UPDATE queries using WHERE.",
        );
      }
  ...
  protected function _updateBatch(string $table, array $keys, array $values): string
  {
    ...
    if ($sql === "") {
      ...
      if ($constraints === []) {
        if ($this->db->DBDebug) {
          throw new DatabaseException(
            "You must specify a constraint to match on for batch updates.",
          ); // @codeCoverageIgnore
        }
      ...
      $sql .=
        implode(
          ",\n",
          array_map(
            static fn($key, $value): string => $key .
              ($value instanceof RawSql ? " = " . $value : " = " . $alias . "." . $value),
            array_keys($updateFields),
            $updateFields,
          ),
        ) . "\n";
    ...
    if (isset($this->QBOptions["setQueryAsData"])) {
      ...
      $data =
        implode(
          " UNION ALL\n",
          array_map(
            static fn($value): string => "SELECT " .
              implode(
                ", ",
                array_map(static fn($key, $index): string => $index . " " . $key, $keys, $value),
              ),
            $values,
          ),
        ) . "\n";
  ...
  protected function _deleteBatch(string $table, array $keys, array $values): string
  {
    ...
    if ($sql === "") {
      ...
      if ($constraints === []) {
        if ($this->db->DBDebug) {
          throw new DatabaseException(
            "You must specify a constraint to match on for batch deletes.",
          ); // @codeCoverageIgnore
        }
      ...
      foreach ($this->QBWhere as $key => $where) {
        foreach ($this->binds as $field => $bind) {
          $this->QBWhere[$key]["condition"] = str_replace(
            ":" . $field . ":",
            $bind[0],
            $where["condition"],
          );
      ...
      foreach ($this->{$qbKey} as &$qbkey) {
        ...
        foreach ($conditions as &$condition) {
          ...
          if ($matches[4] !== "") {
            ...
            if (!str_contains($matches[4], ":")) {
              $matches[4] = $this->db->protectIdentifiers(
                trim($matches[4]),
                false,
                $protectIdentifiers,
              );
          ...
          $condition =
            $matches[1] .
            $this->db->protectIdentifiers(trim($matches[2])) .
            " " .
            trim($matches[3]) .
            $matches[4] .
            $matches[5];
  ...
  protected function compileGroupBy(): string
  {
    if (!empty($this->QBGroupBy)) {
      foreach ($this->QBGroupBy as &$groupBy) {
        ...
        $groupBy =
          $groupBy["escape"] === false || $this->isLiteral($groupBy["field"])
            ? $groupBy["field"]
  ...
  protected function getOperator(string $str, bool $list = false)
  {
    if ($this->pregOperators === []) {
      $_les =
        $this->db->likeEscapeStr !== ""
          ? "\s+" .
            preg_quote(trim(sprintf($this->db->likeEscapeStr, $this->db->likeEscapeChar)), "/")
          : "";
      $this->pregOperators = [
        "\s*(?:<|>|!)?=\s*", // =, <=, >=, !=
        "\s*<>?\s*", // <, <>
    ...
    return preg_match_all("/" . implode("|", $this->pregOperators) . "/i", $str, $match) >= 1
      ? ($list
        ? $match[0]
        : $match[0][0])
      : false;
  ...
  private function getOperatorFromWhereKey(string $whereKey)
  {
    ...
    return preg_match_all("/" . implode("|", $pregOperators) . "/i", $whereKey, $match) >= 1
      ? $match[0]
      : false;
  }
  ...
  protected function buildSubquery($builder, bool $wrapped = false, string $alias = ""): string
  {
    ...
    if ($builder === $this) {
      throw new DatabaseException(
        "The subquery cannot be the same object as the main query object.",
      );
    }
    ...
    if ($wrapped) {
      ...
      if ($alias !== "") {
        $subquery .=
          " " . ($this->db->protectIdentifiers ? $this->db->escapeIdentifiers($alias) : $alias);
      }
    }
