...
class Builder extends BaseBuilder
{
  ...
  protected function _updateBatch(string $table, array $keys, array $values): string
  {
    ...
    if (
      count($constraints) > 1 ||
      isset($this->QBOptions["setQueryAsData"]) ||
      current($constraints) instanceof RawSql
    ) {
      throw new DatabaseException(
        "You are trying to use a feature which requires SQLite version 3.33 or higher.",
      );
    }
    ...
    if ($sql === "") {
      ...
      if (strtolower($alias) !== "`excluded`") {
        throw new InvalidArgumentException(
          'SQLite alias is always named "excluded". A custom alias cannot be used.',
        );
      }
