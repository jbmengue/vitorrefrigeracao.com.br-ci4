...
class Connection extends BaseConnection
{
    ...
    foreach ($parameters as $parameter) {
      ...
      if (in_array($key, $allowedParams, true)) {
        if ($previousParameter !== "") {
          if (
            array_search($key, $allowedParams, true) <
            array_search($previousParameter, $allowedParams, true)
          ) {
            $output .= ";";
          } else {
            $output .= " ";
  ...
  public function getVersion(): string
  {
    ...
    $this->dataCache["version"] = isset($pgVersion["server"])
      ? (preg_match("/^(\d+\.\d+)/", $pgVersion["server"], $matches)
        ? $matches[1]
        : "")
      : "";
  ...
  {
    $sql =
      'SELECT "column_name", "data_type", "character_maximum_length", "numeric_precision", "column_default",  "is_nullable"
            FROM "information_schema"."columns"
            WHERE LOWER("table_name") = ' .
