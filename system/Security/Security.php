...
class Security implements SecurityInterface
{
  ...
  public function verify(RequestInterface $request)
  {
    ...
    try {
      $token =
        $postedToken !== null && $this->config->tokenRandomize
  ...
  {
    $this->cookie = new Cookie($this->rawCookieName, $this->hash, [
      "expires" =>
        $this->config->expires === 0 ? 0 : Time::now()->getTimestamp() + $this->config->expires,
    ]);
