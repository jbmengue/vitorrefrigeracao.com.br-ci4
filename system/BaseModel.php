...
abstract class BaseModel
{
  ...
  {
    if ($this->useCasts()) {
      $this->converter = new DataConverter($this->casts, $this->castHandlers, $this->db);
    }
  }
  ...
  protected function setCreatedField(array $row, $date): array
  {
    if (
      $this->useTimestamps &&
      $this->createdField !== "" &&
      !array_key_exists($this->createdField, $row)
    ) {
  ...
  protected function setUpdatedField(array $row, $date): array
  {
    if (
      $this->useTimestamps &&
      $this->updatedField !== "" &&
      !array_key_exists($this->updatedField, $row)
    ) {
      ...
      foreach ($set as &$row) {
        ...
        if ($updateIndex === null) {
          throw new InvalidArgumentException(
            'The index ("' .
              $index .
              '") for updateBatch() is missing in the data: ' .
              json_encode($row),
          );
        }
