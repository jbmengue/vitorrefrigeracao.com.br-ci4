...
class Forge extends BaseForge
{
  ...
  protected function _createTableAttributes(array $attributes): string
  {
    ...
    if (
      $this->db->charset !== "" &&
      !str_contains($sql, "CHARACTER SET") &&
  ...
  protected function _alterTable(string $alterType, string $table, $processedFields)
  {
    ...
    foreach ($processedFields as $i => $field) {
      if ($field["_literal"] !== false) {
        ...
        $processedFields[$i] =
          $processedFields[$i]["_literal"] . $this->_processColumn($processedFields[$i]);
      }
    }
  ...
  {
    $extraClause = isset($processedField["after"])
      ? " AFTER " . $this->db->escapeIdentifiers($processedField["after"])
      : "";

    if (
      empty($extraClause) &&
      isset($processedField["first"]) &&
      $processedField["first"] === true
    ) {
      $extraClause = " FIRST";
    }
