...
class FileLocator implements FileLocatorInterface
{
  ...
  public function getClassname(string $file): string
  {
    ...
    foreach ($tokens as $i => $token) {
      ...
      if (
        ...
        ($dlm && $tokens[$i - 1][0] === T_NS_SEPARATOR && $token[0] === T_STRING)
      ) {
        if (!$dlm) {
          $namespace = "";
        }
  ...
  public function findQualifiedNameFromPath(string $path)
  {
    ...
    foreach ($this->getNamespaces() as $namespace) {
      ...
      $namespace["path"] =
        $resolvedNamespacePath !== false ? $resolvedNamespacePath : $namespace["path"];
      ...
      if (mb_strpos($path, $namespace["path"]) === 0) {
        $className =
          $namespace["prefix"] .
          "\\" .
          ltrim(str_replace("/", "\\", mb_substr($path, mb_strlen($namespace["path"]))), "\\");
