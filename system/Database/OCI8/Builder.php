...
class Builder extends BaseBuilder
{
  ...
  protected function _insertBatch(string $table, array $keys, array $values): string
  {
    ...
    if ($sql === "") {
      ...
      $hasPrimaryKey = in_array(
        "PRIMARY",
        array_column($this->db->getIndexData($table), "type"),
        true,
      );
  ...
  protected function _replace(string $table, array $keys, array $values): string
  {
    ...
    $replaceableFields = array_filter($keys, static function ($columnName) use (
      $uniqueIndexes,
    ): bool {
    ...
    $sql .= implode(
      ", ",
      array_map(
        static fn($columnName, $value): string => $value . " " . $columnName,
        $keys,
        $values,
      ),
    );
    ...
    foreach ($uniqueIndexes as $index) {
      $onList[] =
        ...
        implode(
          " AND ",
          array_map(
            static fn($columnName): string => $table .
              '."' .
              $columnName .
              '" = "_replace"."' .
              $columnName .
    ...
    $sql .= implode(
      ", ",
      array_map(
        static fn($columnName): string => $columnName . ' = "_replace".' . $columnName,
        $replaceableFields,
      ),
    );
    ...
    return $sql .
      (" (" .
        implode(
          ", ",
          array_map(
            static fn($columnName): string => '"_replace".' . $columnName,
            $replaceableFields,
          ),
        ) .
        ")");
  ...
  protected function _upsertBatch(string $table, array $keys, array $values): string
  {
    ...
    if ($sql === "") {
      ...
      if (empty($constraints)) {
        ...
        $uniqueIndexes = array_filter($this->db->getIndexData($table), static function (
          $index,
        ) use ($fieldNames): bool {
          $hasAllFields =
            count(array_intersect($index->fields, $fieldNames)) === count($index->fields);
      ...
      $sql .=
        ...
        implode(
          ", ",
          array_map(static fn($columnName): string => "{$alias}.{$columnName}", $keys),
        ) .
        ")";
