...
class Builder extends BaseBuilder
{
  ...
  protected function _update(string $table, array $values): string
  {
    ...
    $statement = sprintf(
      "UPDATE %s%s SET ",
      empty($this->QBLimit) ? "" : "TOP(" . $this->QBLimit . ") ",
      $fullTableName,
    );
  ...
  public function increment(string $column, int $value = 1)
  {
    ...
    if ($this->castTextToInt) {
      $values = [
        $column => "CONVERT(VARCHAR(MAX),CONVERT(INT,CONVERT(VARCHAR(MAX), {$column})) + {$value})",
      ];
    } else {
  ...
  protected function _replace(string $table, array $keys, array $values): string
  {
    ...
    $escKeyFields = array_map(
      fn(string $field): string => $this->db->protectIdentifiers($field),
  ...
  protected function _upsertBatch(string $table, array $keys, array $values): string
  {
    ...
    if ($sql === "") {
      ...
      if (empty($constraints)) {
        ...
        $uniqueIndexes = array_filter($tableIndexes, static function ($index) use (
          $fieldNames,
        ): bool {
          $hasAllFields =
